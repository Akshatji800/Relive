...
class _AddNewFoodPageState extends State<AddNewFoodPage> {
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          Fluttertoast.showToast(
                              msg: "Saved Food Nutrients Data", gravity: ToastGravity.TOP);
                        },
                        child: Container(
                          height: 50,
                          margin: const EdgeInsets.symmetric(horizontal: 75),
  ...
  Future<void> updateData() async {
    ...
    if(widget.callingText == "dinner"){
      await DatabaseService(uid: user.uid).updateFoodDataDinner(foodName, calories, carbo, protein, fat, sugars, cholesterol, quantity);
    }
    if(widget.callingText == "lunch"){
      await DatabaseService(uid: user.uid).updateFoodDataLunch(foodName, calories, carbo, protein, fat, sugars, cholesterol, quantity);
    }
    if(widget.callingText == "snack"){
      await DatabaseService(uid: user.uid).updateFoodDataSnack(foodName, calories, carbo, protein, fat, sugars, cholesterol, quantity);
    }
    if(widget.callingText == "breakfast"){
      await DatabaseService(uid: user.uid).updateFoodDataBreakfast(foodName, calories, carbo, protein, fat, sugars, cholesterol, quantity);
    }
  }
}
